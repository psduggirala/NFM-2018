...
%
\new{More specifically, we recompute the reachable set \emph{only for the time intervals at discrete transitions or overlap with unsafe set.}}
%
\new{This selective recomputation is the main difference between this work and~\cite{schupp2018efficient}, where, SpaceEx is invoked with various configuration parameters for reducing the error in reachable set computation.} 
%
\new{To keep track of the aggregation, deaggregation, and time instances for selective recomputation, we maintain a data structure called Aggregated Directed Acyclic Graph (AGGDAG).}
%
