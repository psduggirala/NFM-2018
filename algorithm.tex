...
\While{${\it queueStars}$ is not empty}{ \label{ln:begLoop}
    $S_{agg} \gets {\sf aggregation}($ all aggregatable stars going to same mode in ${\it queueStars})$\;
    $S_{agg}.tag \gets {\sf aggregate}$; dequeue stars going to $S_{agg}.loc$ from $queueStars$\;
    $R \gets {\sf ReachableSetDynamicalSystem}(S_{agg}, S_{agg}.loc)$\;
    $R' \gets {\sf InvariantOverlap}(R, R.loc)$\;
    \If{$R' \cap U \neq \emptyset$ and $R'.tag = {\sf aggregate}$ }{ \label{ln:deagg}
        Deaggregate at least one star in the path from $S_{agg}$ to root\;
        Enqueue the $queueStars$ with the results of deaggregation\;
    }
...
\noindent {\bf \normalsize Template Based Aggregation:}
...
\noindent {\bf \normalsize Convex Hull Aggregation:}
